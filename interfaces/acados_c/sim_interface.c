...
void sim_config_free(void *config) { free(config); }
...
void sim_dims_free(void *dims) { free(dims); }



void sim_dims_set_nx(sim_solver_config *config, void *dims, int nx) { config->set_nx(dims, nx); }



void sim_dims_set_nu(sim_solver_config *config, void *dims, int nu) { config->set_nu(dims, nu); }
...
void sim_in_free(void *in) { free(in); }
...
void sim_in_set_x(sim_solver_config *config, void *dims, double *x, sim_in *in)
{
    ...
    int ii;
    for (ii = 0; ii < nx; ii++) in->x[ii] = x[ii];
    return;
}
...
void sim_in_set_xdot(sim_solver_config *config, void *dims, double *xdot, sim_in *in)
{
    ...
    config->get_nx(dims, &nx);
    int ii;
    for (ii = 0; ii < nx; ii++) in->xdot[ii] = xdot[ii];
    return;
}
...
void sim_in_set_u(sim_solver_config *config, void *dims, double *u, sim_in *in)
{
    ...
    int ii;
    for (ii = 0; ii < nu; ii++) in->u[ii] = u[ii];
    return;
}
...
void sim_in_set_Sx(sim_solver_config *config, void *dims, double *Sx, sim_in *in)
{
    ...
    config->get_nx(dims, &nx);
    int ii;
    for (ii = 0; ii < nx * nx; ii++) in->S_forw[ii] = Sx[ii];
    return;
}
...
void sim_out_free(void *out) { free(out); }
...
void sim_out_get_xn(sim_solver_config *config, void *dims, sim_out *out, double *xn)
{
    ...
    config->get_nx(dims, &nx);
    int ii;
    for (ii = 0; ii < nx; ii++) xn[ii] = out->xn[ii];
    return;
}
...
void sim_out_get_Sxn(sim_solver_config *config, void *dims, sim_out *out, double *Sxn)
{
    ...
    config->get_nx(dims, &nx);
    int ii;
    for (ii = 0; ii < nx * nx; ii++) Sxn[ii] = out->S_forw[ii];
    return;
}
...
void sim_opts_free(void *opts) { free(opts); }
...
void sim_free(void *solver) { free(solver); }
...
{
    int flag =
        solver->config->evaluate(solver->config, in, out, solver->opts, solver->mem, solver->work);
    return flag;
}
